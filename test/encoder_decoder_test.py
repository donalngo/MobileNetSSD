...
# train_dataset, train_imgs = datagen.data_generator(img_dir=train_image_dir, xml_dir=train_image_dir,
#                                                    batch_size=batch_size, steps_per_epoch=None, img_sz=224,
#                                                    label_encoder=label_encoder,
#                                                    translate=0, rotate=0, scale=1, shear=0, hor_flip=False,
#                                                    ver_flip=False)
# val_dataset, val_imgs = datagen.data_generator(img_dir=val_image_dir, xml_dir=val_image_dir, batch_size=batch_size,
#                                                steps_per_epoch=None, img_sz=224, label_encoder=label_encoder,
...
model_checkpoint = ModelCheckpoint(
    filepath='pretrain_ssd224+epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
    monitor='val_loss',
    verbose=1,
    save_best_only=True,
...
for i in range(len(ImgList)):
    ...
    print('y_truth returned from label_encoder', y_truth)
...
for box in y_pred[CurrentSelect][0]:
    xmin = box[2]
    ymin = box[3]
    xmax = box[4]
    ymax = box[5]
